...
- [ ] Test-drive a new class, `DatabaseConnection`, with two methods:
  ...
  - [ ] `DatabaseConnection.query` is a class method. It takes two parameters: an SQL query string and an optional array. It should use the class instance variable from `setup` to execute that SQL query string on the correct database, via `pg`.
